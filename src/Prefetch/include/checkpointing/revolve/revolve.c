 ...
 *   refer to as                                                      *
 *                                                                    *
 *       'revolve_advance', 'takeshot', 'restore', 'firsturn' and 'youturn'  .*
 *   There are two other return values, namely                        *
 *       'revolve_terminate'   and     'revolve_error'                                *
 ...
 *                   and contains information about an revolve_error occured  *
 *                                                                    *
 ...
 *   If either condition is violated the value 'revolve_error' is returned.   *
 *                                                                    *
 *   The first call to REVOLVE must be with CHECK=-1 so that          *
 *   appropriate initializations can be performed internally.         *
 *                                                                    *
 *   When CHECK =-1 and CAPO = FINE  then 'revolve_terminate' is returned as  *
 ...
 *   down. When an revolve_error occurs, the return value of INFO contains    *
 ...
 *     INFO = 12: revolve_error occurs in NUMFORW                             *
 ...
 *   for then control is returned after an appropriate revolve_error message. *
 *   When the calculated REPS exceeds REPSUP also an revolve_error message    *
 *   occurs.                                                          *
 ...
 *   revolve_error message will be given and -1 is returned as value.         *
 *                                                                    *
 *--------------------------------------------------------------------*
 *                                                                    *
 ...
 *   SNAPS >=0, REPS >= 0. Otherwise there will be appropriate revolve_error  *
...
unsigned max_revolve_checkpoints() { return checkup; }
...
int numforw(int steps, int snaps) {
  ...
  if (snaps < 1) {
    printf(" revolve_error occurs in numforw: snaps < 1\n");
    return -1;
  }
  if (snaps > checkup) {
...
double expense(int steps, int snaps) {
  ...
  if (snaps < 1) {
    printf(" revolve_error occurs in expense: snaps < 0\n");
    return -1;
  }
  if (steps < 1) {
    printf(" revolve_error occurs in expense: steps < 0\n");
    return -1;
  }
...
int maxrange(int ss, int tt) {
  ...
  if ((tt < 0) || (ss < 0)) {
    printf("revolve_error in MAXRANGE: negative parameter");
    return -1;
  }
  for (i = 1; i <= tt; i++) {
    res *= (ss + i);
